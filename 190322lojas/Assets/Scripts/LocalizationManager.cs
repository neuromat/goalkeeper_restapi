...
//	private GameObject localizationScreen;               //171004 language selection screen
	...
	IEnumerator readLocalizedText(string language)
	{
		...
		Debug.Log ("isRunningTrue " + isRunning);
		...
		isRunning = false;
		Debug.Log ("isRunningFalse " + isRunning);
		...
		if (!error) {         //found the file, let's read
			...
			for (int i = 0; i < keyValueArray.Length; i++) {	
				...
				//Debug.Log ("isRunning i " + isRunning + " i : " +i);
    ...
    {	
		if (localizedText.ContainsKey (key)) {
			//Debug.Log ("localizedText [key] " + localizedText [key] + " key = " + key);
